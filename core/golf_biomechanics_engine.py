...
class GolfBiomechanicsEngine:
    ...
    def sample_frames(self, video_path: str, max_frames: int = 8) -> List[np.ndarray]:
    ...
    def analyze_quality(self, frames: List[np.ndarray]) -> Dict[str, Any]:
        if not frames:
    ...
    def heuristic_faults(self, frames: List[np.ndarray]) -> Dict[str, Any]:
    ...
    def llm_swing_analysis(self, frames: List[np.ndarray]) -> Dict[str, Any]:
        if not self.client:
            return {
                "status": "fallback",
        ...
        content = [{
            ...
            "text": (
                ...
                "Evalúa setup, backswing, top, transición, downswing, impacto, release, finish, balance, secuencia, y explica errores fáciles de entender. "
        ...
        try:
            ...
            return {
                "status": "error",
                "summary": f"Falló análisis avanzado: {exc}"
            }
