...
class GolfAIAgent:
    ...
    def __init__(self):
        ...
        self.vision = GolfSwingVisionPro()
    def __init__(self) -> None:
        self.db = GolfCourseDatabase()
        self.bio = GolfBiomechanicsEngine()
    ...
    def compare_swings(self, video_path_a: str, video_path_b: str, label_a: str = "before", label_b: str = "after") -> Dict[str, Any]:
    ...
    def fitting_recommendation(self, player_profile: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if common_miss in ["hook", "izquierda"]:
    ...
    def course_caddie(self, latitude: float, longitude: float, hole_number: Optional[int] = None) -> Dict[str, Any]:
        ...
        if not course:
            return {
                "status": "not_found",
                "message": "No encontré un campo cercano en la base local. El sistema está listo para crecer con imports masivos."
            }
    ...
    def search_courses(self, query: str, limit: int = 10) -> Dict[str, Any]:
        return {
            "status": "ok",
            "items": self.db.search_by_name(query, limit=limit)
        }
    ...
    def _club_from_distance(self, d: float) -> str:
        ...
        if d < 193:
            return "5 Iron"
        if d < 208:
            return "4 Iron / Hybrid"
        if d < 228:
            return "5 Wood / Strong Hybrid"
